...
def pandas_append(df, row, ignore_index=False):
    ...
    if isinstance(row, pd.DataFrame):
        ...
        result = pd.concat(
            [df, pd.DataFrame(row, index=[0], columns=df.columns)])
    else:
        raise RuntimeError(
            "pandas_append: unsupported row type - {}".format(type(row)))
    return result
...
class DetectObjectsDlg(QtWidgets.QDialog):
    ...
    def __init__(self, parent):
        ...
        if len(Metashape.app.document.path) > 0:
            self.working_dir = str(pathlib.Path(
                Metashape.app.document.path).parent / "objects_detection")
        else:
            self.working_dir = ""
        ...
        # bigger number of epochs leads to better neural network training (but slower)
        self.max_epochs = 20
        # from 1 to 8, bigger multiplier leads to better neural network training (but slower)
        self.data_augmentation_multiplier = 8
        self.preferred_patch_size = 400  # 400 pixels
        self.preferred_resolution = 0.10  # 10 cm/pix
        # can be from 0.0 to 1.0, for example it can be 0.98
        self.detection_score_threshold = None
        ...
        self.tiles_without_annotations_supported = False
    ...
    def prepair(self):
        ...
        if self.working_dir == "":
            raise Exception(
                "You should specify working directory (or save .psx project)")
        ...
        try:
            ...
            print("Working dir: {} already exists, trying instead: {}".format(
                already_existing_working_dir, self.working_dir))
            try:
                os.mkdir(self.working_dir)
            except FileExistsError:
                raise Exception(
                    "Working directory {} already exists! Please specify another working dir.".format(self.working_dir))
    ...
    def create_neural_network(self):
        ...
        if len(self.load_model_path) > 0:
            self.m.use_release()
            print("Using the neural network loaded from '{}'...".format(
    ...
    def export_orthomosaic(self):
        ...
        for tile in sorted(tiles):
            ...
            if tile.endswith(".jgw") or tile.endswith(".pgw"):
        ...
        print("{} tiles, tile_x in [{}; {}], tile_y in [{}; {}]".format(len(
            self.tiles_paths), self.tile_min_x, self.tile_max_x, self.tile_min_y, self.tile_max_y))
    ...
    def train_on_user_data(self):
        ...
        for zone_i, shape in enumerate(self.train_zones):
            ...
            for tile_x in range(self.tile_min_x, self.tile_max_x):
                for tile_y in range(self.tile_min_y, self.tile_max_y):
                    ...
                    for p in shape_vertices:
                        p = Metashape.CoordinateSystem.transform(
                            p, self.chunk.shapes.crs, self.chunk.orthomosaic.crs)
                        ...
                        if zone_from_world_best is None or distance2_to_tile_center < zone_from_world_best:
                            ...
                            zone_from_world = self.invert_matrix_2x3(self.add_pixel_shift(
                                to_world, -tile_x * self.patch_size, -tile_y * self.patch_size))
            ...
            for p in shape_vertices:
                p = Metashape.CoordinateSystem.transform(
                    p, self.chunk.shapes.crs, self.chunk.orthomosaic.crs)
                p_in_ortho = np.int32(
                    np.round(zone_from_world @ [p.x, p.y, 1]))
            ...
            train_size_m = np.int32(
                np.round(train_size * self.orthomosaic_resolution))
            if np.any(train_size < self.patch_size):
                ...
                print("Train zone #{}: {}x{} orthomosaic pixels, {}x{} meters".format(
                    zone_i + 1, train_size[0], train_size[1], train_size_m[0], train_size_m[1]))
                self.train_zones_on_ortho.append(
                    (zone_from, zone_to, zone_from_world))
        assert len(self.train_zones_on_ortho) == len(self.train_zones)

        if n_train_zone_shapes_out_of_orthomosaic > 0:
            print("Warning, {} train zones shapes are out of orthomosaic".format(
                n_train_zone_shapes_out_of_orthomosaic))
        ...
        if self.tiles_without_annotations_supported:
            ...
            # See https://github.com/weecology/DeepForest/issues/216
            all_annotations = pandas_append(all_annotations, {
                                            'image_path': empty_tile_name, 'xmin': '0', 'ymin': '0', 'xmax': '0', 'ymax': '0', 'label': 'Tree'}, ignore_index=True)
        ...
        for zone_i, shape in enumerate(self.train_zones):
            ...
            for annotation in self.train_data:
                ...
                bbox_from, bbox_to = self.intersect(
                    zone_from, zone_to, annotation_from, annotation_to)
            ...
            print("Train zone #{}: {} annotations inside".format(
                zone_i + 1, len(annotations)))
            ...
            nx_tiles, ny_tiles = np.int32(
                (zone_size - 2 * border + inner_path_size - 1) // inner_path_size)
            assert nx_tiles >= 1 and ny_tiles >= 1
            xy_step = np.int32(
                np.round((zone_size + [nx_tiles, ny_tiles] - 1) // [nx_tiles, ny_tiles]))
            out_of_orthomosaic_train_tile = 0
            for x_tile in range(0, nx_tiles):
                for y_tile in range(0, ny_tiles):
                    tile_to = zone_from + self.patch_size + \
                        xy_step * [x_tile, y_tile]
                    ...
                    for annotation_from, annotation_to in annotations:
                        bbox_from, bbox_to = self.intersect(
                            tile_from, tile_to, annotation_from, annotation_to)
                        if self.area(bbox_from, bbox_to) > self.area(annotation_from, annotation_to) * area_threshold:
                            tile_annotations.append(
                                (bbox_from - tile_from, bbox_to - tile_from))

                    max_augmented_versions = 8
                    all_augmented_versions = list(
                        range(max_augmented_versions))

                    augmented_versions = []
                    augmented_versions_to_add = max(
                        1, self.data_augmentation_multiplier)
                    while augmented_versions_to_add > 0:
                        if augmented_versions_to_add < max_augmented_versions:
                            ...
                            augmented_versions.extend(
                                shuffled_augmented_versions[:augmented_versions_to_add])
                    ...
                    for version_i in augmented_versions:
                        ...
                        if is_mirrored:
                            tile_annotations_version = self.flip_annotations(
                                tile_annotations_version, tile_version)
                            tile_version = cv2.flip(tile_version, 0)
                        for rotation_i in range(n90rotation):
                            tile_annotations_version = self.rotate90clockwise_annotations(
                                tile_annotations_version, tile_version)
                            tile_version = cv2.rotate(
                                tile_version, cv2.ROTATE_90_CLOCKWISE)

                        tile_version = self.random_augmentation(tile_version)

                        tile_name = "{}-{}-{}-{}.jpg".format(
                            (zone_i + 1), x_tile, y_tile, version_i)

                        nannotated_tiles += 1
                        for (xmin, ymin), (xmax, ymax) in tile_annotations_version:
                            all_annotations = pandas_append(all_annotations, {
                                                            'image_path': tile_name, 'xmin': xmin, 'ymin': ymin, 'xmax': xmax, 'ymax': ymax, 'label': 'Tree'}, ignore_index=True)
                        if len(tile_annotations_version) == 0:
                            if self.tiles_without_annotations_supported:
                                all_annotations = pandas_append(all_annotations, {
                                                                'image_path': tile_name, 'xmin': '0', 'ymin': '0', 'xmax': '0', 'ymax': '0', 'label': 'Tree'}, ignore_index=True)
                            nempty_tiles += 1

                        cv2.imwrite(self.dir_train_subtiles +
                                    tile_name, tile_version)
                        if self.debug_tiles:
                            tile_with_trees = self.debug_draw_trees(
                                tile_version, tile_annotations_version)
                            cv2.imwrite(
                                self.dir_train_subtiles_debug + tile_name, tile_with_trees)

            if out_of_orthomosaic_train_tile == nx_tiles * ny_tiles:
                raise RuntimeError(
                    "It seems that zone #{} has no orthomosaic data, please check zones, orthomosaic and its Outer Boundary.".format(zone_i + 1))
            else:
                if out_of_orthomosaic_train_tile > 0:
                    print("{}/{} of tiles in zone #{} has no orthomosaic data".format(
                        out_of_orthomosaic_train_tile, nx_tiles * ny_tiles, zone_i + 1))
        ...
        print("Training with {} epochs and x{} augmentations (augment colors: {})...".format(
            self.max_epochs, self.data_augmentation_multiplier, self.augment_colors))
        self.freeze_layers()
        ...
        trainer = Trainer(max_epochs=self.max_epochs, gpus=trainer_gpus, auto_select_gpus=trainer_auto_select_gpus, callbacks=[
                          MyCallback(self)], checkpoint_callback=False, logger=False)
        train_ds = self.m.load_dataset(
            annotations_file, root_dir=os.path.dirname(annotations_file))
        ...
        if len(self.save_model_path) > 0:
            ...
            print("Model trained on {} annotations with {} m/pix resolution saved to '{}'".format(
                self.train_nannotations_in_zones, self.orthomosaic_resolution, self.save_model_path))
    ...
    def random_augmentation(self, img):
        ...
        if self.augment_colors:
            stages.append(A.HueSaturationValue(
                hue_shift_limit=360, sat_shift_limit=30, val_shift_limit=20, always_apply=True))
        stages.append(A.ISONoise(p=0.5))
        if self.augment_colors:
            stages.append(A.RandomBrightnessContrast(
                brightness_limit=0.4, contrast_limit=0, p=0.5))
        ...
        for tile_x in range(tile_xy_from[0], tile_xy_upto[0] + 1):
            for tile_y in range(tile_xy_from[1], tile_xy_upto[1] + 1):
                ...
                part = cv2.copyMakeBorder(
                    part, 0, self.patch_size - part.shape[0], 0, self.patch_size - part.shape[1], cv2.BORDER_CONSTANT, value=[255, 255, 255])
                part_from = np.int32([tile_x, tile_y]) * \
                    self.patch_size - res_from
                part_to = part_from + self.patch_size

                res_inner_from = np.int32(
                    [max(0, part_from[0]), max(0, part_from[1])])
                res_inner_to = np.int32(
                    [min(part_to[0], res_size[0]), min(part_to[1], res_size[1])])

                part_inner_from = res_inner_from - part_from
                part_inner_to = part_inner_from + res_inner_to - res_inner_from

                res[res_inner_from[1]:res_inner_to[1], res_inner_from[0]:res_inner_to[0],
                    :] = part[part_inner_from[1]:part_inner_to[1], part_inner_from[0]:part_inner_to[0], :]
    ...
    def detect(self):
        ...
        detected_label = self.layer_name_detection_data + \
            " ({:.2f} cm/pix".format(100.0 * self.orthomosaic_resolution)
        if self.use_neural_network_pretrained_on_birds:
            detected_label += ", birds pre-train"
        if self.augment_colors:
            detected_label += ", any color"
        if self.train_on_user_data_enabled:
            detected_label += ", trained on {} shapes in {} zones".format(
                self.train_nannotations_in_zones, len(self.train_zones))
        detected_label += ")"
        ...
        for big_tile_index, (big_tile_x, big_tile_y) in enumerate(sorted(big_tiles)):
            big_tile = np.zeros((border + big_tiles_k*self.patch_size + border,
                                border + big_tiles_k*self.patch_size + border, 3), np.uint8)
            big_tile[:, :, :] = 255
            ...
            for xi in range(-1, big_tiles_k + 1):
                for yi in range(-1, big_tiles_k + 1):
                    ...
                    if xi in [-1, big_tiles_k] or yi in [-1, big_tiles_k]:
                        ...
                        big_tile[border + yi * self.patch_size:, border + xi *
                                 self.patch_size:, :][:self.patch_size, :self.patch_size, :] = part
                        big_tile_to_world = self.add_pixel_shift(self.tiles_to_world[tile_x, tile_y], -(
                            border + xi * self.patch_size), -(border + yi * self.patch_size))
            ...
            inner_tiles_nx = (
                big_tile.shape[1]-2*border+tile_inner_size-1)//tile_inner_size
            inner_tiles_ny = (
                big_tile.shape[0]-2*border+tile_inner_size-1)//tile_inner_size
            for xi in range(inner_tiles_nx):
                for yi in range(inner_tiles_ny):
                    tox, toy = min(big_tile.shape[1], 2*border+(xi + 1) * tile_inner_size), min(
                        big_tile.shape[0], 2*border+(yi + 1) * tile_inner_size)
                    fromx, fromy = tox - self.patch_size, toy - self.patch_size
                    subtile = big_tile[fromy:toy, fromx:tox, :]

                    white_pixels_fraction = np.sum(
                        np.all(subtile == 255, axis=-1)) / (subtile.shape[0] * subtile.shape[1])

                    assert(subtile.shape == (
                        self.patch_size, self.patch_size, 3))
                    subtile_trees = self.m.predict_image(
                        subtile.astype('float32'))
                    Metashape.app.update()
                    app.processEvents()
                    self.check_stopped()
                    if subtile_trees is not None:
                        subtile_inner_trees = pd.DataFrame(
                            columns=['image_path', 'xmin', 'ymin', 'xmax', 'ymax', 'label'])
                        subtile_inner_trees_debug = pd.DataFrame(
                            columns=['image_path', 'xmin', 'ymin', 'xmax', 'ymax', 'label'])
                        for idx, row in subtile_trees.iterrows():
                            xmin, ymin, xmax, ymax, label, score = int(row.xmin), int(
                                row.ymin), int(row.xmax), int(row.ymax), row.label, row.score
                            assert (label == "Tree")
                            ...
                            if white_pixels_fraction > 0.10:
                                ...
                                bbox_white_pixels_fraction = np.sum(np.all(
                                    subtile_bbox == 255, axis=-1)) / (subtile_bbox.shape[0] * subtile_bbox.shape[1])
                                if bbox_white_pixels_fraction > 0.70:
                                    continue
                            xmin, xmax = map(lambda x: fromx + x, [xmin, xmax])
                            ymin, ymax = map(lambda y: fromy + y, [ymin, ymax])
                            subtile_inner_trees_debug = pandas_append(
                                subtile_inner_trees_debug, row, ignore_index=True)
                            row.xmin, row.ymin, row.xmax, row.ymax = xmin, ymin, xmax, ymax
                            subtile_inner_trees = pandas_append(
                                subtile_inner_trees, row, ignore_index=True)

                        if self.debug_tiles:
                            img_with_trees = self.debug_draw_trees(
                                subtile, subtile_trees)
                            cv2.imwrite(self.dir_subtiles_results + "{}-{}-{}-{}.jpg".format(
                                big_tile_x, big_tile_y, xi, yi), img_with_trees)
                            img_with_inner_trees = self.debug_draw_trees(
                                subtile, subtile_inner_trees_debug)
                            cv2.imwrite(self.dir_subtiles_results + "{}-{}-{}-{}_inner.jpg".format(
                                big_tile_x, big_tile_y, xi, yi), img_with_inner_trees)
                    else:
                        subtile_inner_trees = pd.DataFrame(
                            columns=['image_path', 'xmin', 'ymin', 'xmax', 'ymax', 'label'])
                        if self.debug_tiles:
                            cv2.imwrite(self.dir_subtiles_results + "{}-{}-{}-{}_empty.jpg".format(
                                big_tile_x, big_tile_y, xi, yi), subtile)
            ...
            for xi, yi in sorted(subtiles_trees.keys()):
                ...
                for idx, rowA in a.iterrows():
                    axmin, aymin, axmax, aymax, ascore = int(rowA.xmin), int(
                        rowA.ymin), int(rowA.xmax), int(rowA.ymax), rowA.score
                    ...
                    for dy in [-1, 0, 1]:
                        ...
                        for idx in indices_to_check:
                            ...
                            for _, rowB in b.iterrows():
                                bxmin, bymin, bxmax, bymax, bscore = int(rowB.xmin), int(
                                    rowB.ymin), int(rowB.xmax), int(rowB.ymax), rowB.score
                                areaB = (bxmax - bxmin) * (bymax - bymin)

                                intersectionx = max(
                                    0, min(axmax, bxmax) - max(axmin, bxmin))
                                intersectiony = max(
                                    0, min(aymax, bymax) - max(aymin, bymin))
                ...
                for idx, row in a.iterrows():
                    if row.label == "Suppressed":
                        continue
                    big_tile_trees = pandas_append(
                        big_tile_trees, row, ignore_index=True)
            ...
            if self.debug_tiles:
                cv2.imwrite(self.dir_detection_results +
                            "{}-{}_clean.jpg".format(big_tile_x, big_tile_y), big_tile)
                img_with_trees = self.debug_draw_trees(
                    big_tile, big_tile_trees)
                cv2.imwrite(self.dir_detection_results +
                            "{}-{}_all_trees.jpg".format(big_tile_x, big_tile_y), img_with_trees)
                img_with_border_trees = self.debug_draw_trees(
                    big_tile, big_tile_trees.loc[idx_on_borders])
                cv2.imwrite(self.dir_detection_results + "{}-{}_border_trees.jpg".format(
                    big_tile_x, big_tile_y), img_with_border_trees)

            self.detectionPBar.setValue(
                (big_tile_index + 1) * 100 / len(big_tiles))
            Metashape.app.update()
            app.processEvents()
            self.check_stopped()
        ...
        for big_tile_x, big_tile_y in sorted(big_tiles):
                    ...
                    for idxA in a_idx_on_borders:
                        ...
                        for idxB in b_idx_on_borders:
                            ...
                            bxmin, bymin, bxmax, bymax, bscore = int(rowB.xmin), int(
                                rowB.ymin), int(rowB.xmax), int(rowB.ymax), rowB.score
                            bxmin, bxmax = map(
                                lambda x: x + dx * big_tiles_k * self.patch_size, [bxmin, bxmax])
                            bymin, bymax = map(
                                lambda y: y + dy * big_tiles_k * self.patch_size, [bymin, bymax])
                            areaB = (bxmax - bxmin) * (bymax - bymin)
            ...
            if self.debug_tiles:
                big_tile = cv2.imread(
                    self.dir_detection_results + "{}-{}_clean.jpg".format(big_tile_x, big_tile_y))
                img_with_trees = self.debug_draw_trees(
                    big_tile, big_tile_trees)
                cv2.imwrite(self.dir_detection_results + "{}-{}_after_suppression.jpg".format(
                    big_tile_x, big_tile_y), img_with_trees)

            ntrees_detected += len(big_tile_trees)
            self.add_trees(big_tile_to_world, big_tile_trees,
                           detected_shapes_layer)
    ...
    def add_trees(self, to_world, tile_trees, shapes_group):
        ...
        for row in tile_trees.itertuples():
            xmin, ymin, xmax, ymax, label = int(row.xmin), int(
                row.ymin), int(row.xmax), int(row.ymax), row.label
            assert (label == "Tree")
            ...
            for x, y in [(xmin, ymin), (xmax, ymin), (xmax, ymax), (xmin, ymax)]:
                ...
                p = Metashape.CoordinateSystem.transform(
                    p, self.chunk.orthomosaic.crs, self.chunk.shapes.crs)
    ...
    def create_gui(self):
        ...
        self.noTrainDataChoice = (
            None, "No additional training (use pre-trained model as is)", True)
        ...
        if slow_shape_layers_enumerating_but_with_number_of_shapes:
            ...
            for shape in shapes:
                ...
                if layer.key not in self.layersDict:
                    self.layersDict[layer.key] = (
                        layer.key, layer.label, layer.enabled)
                    self.layersSize[layer.key] = 1
                else:
                    self.layersSize[layer.key] += 1
            print("Found {} shapes layers in {:.2f} sec:".format(
                len(self.layersDict), time.time() - shapes_enumerating_start))
            for key in sorted(self.layersDict.keys()):
                key, label, enabled = self.layersDict[key]
                size = self.layersSize[key]
                print("Shape layer: {} shapes, key={}, label={}".format(
                    size, key, label))
                if label == '':
                    label = 'Layer'
            ...
            if self.chunk.shapes is None:
                ...
                for layer in self.chunk.shapes.groups:
                    ...
                    print("Shape layer: key={}, label={}, enabled={}".format(
                        key, label, enabled))
                    if label == '':
                        label = 'Layer'
        ...
        self.chkUse10cmResolution = QtWidgets.QCheckBox(
            "Process with 10 cm/pix resolution (for speedup)")
        self.chkUse10cmResolution.setToolTip(
            "Process with downsampling to 10 cm/pix instad of original orthomosaic resolution. Leads to faster processing.")
        self.chkUse10cmResolution.setChecked(
            not self.prefer_original_resolution)
        ...
        self.edtWorkingDir.setPlaceholderText(
            "Path to dir for intermediate data")
        ...
        QtCore.QObject.connect(self.btnWorkingDir, QtCore.SIGNAL(
            "clicked()"), lambda: self.choose_working_dir())
        ...
        self.groupBoxModelTraining = QtWidgets.QGroupBox(
            "Additional model training (recommended to train at least on a 50x50m zone)")
        ...
        self.groupBoxModelSaveLoad = QtWidgets.QGroupBox(
            "Save/Load trained model (optional, note that orthomosaic resolution should be the same)")
        ...
        self.edtModelSavePath.setPlaceholderText(
            "File for neural network model to be saved after additional training")
        self.edtModelSavePath.setToolTip(
            "File for neural network model to be saved after additional training")
        self.btnModelSavePath = QtWidgets.QPushButton("...")
        self.btnModelSavePath.setFixedSize(25, 25)
        QtCore.QObject.connect(self.btnModelSavePath, QtCore.SIGNAL(
            "clicked()"), lambda: self.choose_model_save_path())
        ...
        self.edtModelLoadPath.setPlaceholderText(
            "File with previously saved neural network model (resolution must be the same)")
        self.edtModelLoadPath.setToolTip(
            "File with previously saved neural network model (resolution must be the same)")
        self.btnModelLoadPath = QtWidgets.QPushButton("...")
        self.btnModelLoadPath.setFixedSize(25, 25)
        QtCore.QObject.connect(self.btnModelLoadPath, QtCore.SIGNAL(
            "clicked()"), lambda: self.choose_model_load_path())
        ...
        QtCore.QObject.connect(self.btnRun, QtCore.SIGNAL(
            "clicked()"), lambda: self.process())
        QtCore.QObject.connect(self.btnStop, QtCore.SIGNAL(
    ...
    def choose_model_save_path(self):
        models_dir = ""
        load_path = Metashape.app.settings.value(
            "scripts/detect_objects/model_load_path")
        if load_path is not None:
            models_dir = str(pathlib.Path(load_path).parent)

        save_path = Metashape.app.getSaveFileName(
            "Trained model save path", models_dir, ".model")
    ...
    def choose_model_load_path(self):
        load_path = Metashape.app.getOpenFileName(
            "Trained model load path", "", "*.model")
    ...
    def saveToSettingsModelLoadPath(self, load_path):
        Metashape.app.settings.setValue(
            "scripts/detect_objects/model_load_path", load_path)
    ...
    def load_params(self):
        ...
        if not self.prefer_original_resolution:
            ...
            if self.orthomosaic_resolution > 0.105:
                raise Exception(
                    "Orthomosaic should have resolution <= 10 cm/pix.")
            if self.force_small_patch_size:
                patch_size_multiplier = 1
            else:
                patch_size_multiplier = max(
                    1, min(4, self.preferred_resolution / self.orthomosaic_resolution))
            self.patch_size = round(
                self.preferred_patch_size * patch_size_multiplier)

        self.patch_inner_border = self.patch_size // 8
        print("Using resolution {} m/pix with patch {}x{}".format(
            self.orthomosaic_resolution, self.patch_size, self.patch_size))
        ...
        if trainZonesLayer == self.noTrainDataChoice or trainDataLayer == self.noTrainDataChoice:
            ...
            print("Additional neural network training expected on key={} layer data w.r.t. key={} layer zones".format(
            ...
            print("{} train zones and {} train data loaded in {:.2f} sec".format(len(
                self.train_zones), len(self.train_data), time.time() - loading_train_shapes_start))
